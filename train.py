...
def main(model_name="maskrcnn_resnet50_fpn_v2", batch_size=32, depth=False, image_size=1280):
    if "yolo" in model_name.lower() or "rtdetr" in model_name.lower():

        # Load a model
        if "yolo" in model_name.lower():
            model = YOLO(model_name,task="segment")  # load a pretrained model (recommended for training)
        elif "rtdetr" in model_name.lower():
            model = RTDETR(model_name)
        data = os.path.join(os.path.abspath(os.path.dirname(__file__)),"M18K/Data/data.yaml")
        # Train the model
        model.train(
                ...
                scale=0.1,
                cache=False,
                name=f"{model_name}_{image_size}"
            )
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, default=1280, help='Image size (default: 1280)')
    parser.add_argument('--depth', default=False, action='store_true', help='include depth (set this flag to use depth)')
    args = parser.parse_args()
    model = args.model
    batch_size = args.batch_size
    image_size = args.image_size
    depth = args.depth
    main(model, batch_size, depth,image_size)
